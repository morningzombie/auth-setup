...
const sync = async()=> {
  ...
  INSERT INTO roles (id, users_id, role_name)
  VALUES
  (uuid_generate_v4(), (SELECT id FROM users where users.name = 'moe'), 
  'ADMIN'),
  (uuid_generate_v4(), (SELECT id FROM users where users.name = 'lucy'), 
  'USER'),
  (uuid_generate_v4(), (SELECT id FROM users where users.name = 'curly'), 
  'USER'),
  `;
